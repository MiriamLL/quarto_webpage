...
description: |
  In this blog post, we’ll explore how to select and filter specific features from a layer and add labels to each polygon in QGIS.
author:
  - name: Miriam Lerma
    url: {}
date: 2026-05-25
...
In this post, we will include offshore wind farms (OWF) in the Baltic Sea, along with their names, for this, the post will focus on adding a vector layer, labeling using the name of each polygon, and add the respective legend on the final map.
...
In the expression window, enter the following query:
...
In addition, you will probably see a blue section indicating how many matching ones have been selected.  
...
To improve clarity, arrange the categories in the following order: Production, Construction, Approved, and then Planned.  This provides a more intuitive overview of the OWF status.  
I added each category as a separated legend to allow better control over the font size.
...
My map was inspired by the [4c offshore map](https://map.4coffshore.com/offshorewind/).  
