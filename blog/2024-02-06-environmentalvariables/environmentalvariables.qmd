...
ggsave(ggplot(SST03.2017dt) +
  geom_tile(aes(x=longitude, y=latitude, fill = sst))+
  geom_polygon(data = world.df, aes(x = long, y = lat, group = group), colour = '#403d39', fill = "#e5e5e5") +
  theme(
    axis.title = element_blank(),
    panel.border = element_rect(colour = "#495057", fill=NA, size=1),
    ...
    coord_sf(xlim = c(xlim1+2,xlim2-2), 
             ylim = c(ylim1+2,ylim2-2))+ 
  scale_fill_viridis_c(option = "H"), 
       filename = paste0(here(),"/blog/2024-02-06-environmentalvariables/environmental_export.jpg"),
       width = 6, 
       height = 3, 
       units = "in",
       dpi = 300)
...
I hope the server did not failed you, sometimes is not working, also welcome geom_tile!
I was using geom_raster before, which it works but why not joining the new way?
